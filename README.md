...
5. Now create 2 more route tables, one for each app layer private subnet in each availability zone. These route tables will route app layer traffic destined for outside the VPC to the NAT gateway in the respective availability zone, so add the appropriate routes for that.
   ...
   ![](/demos/PrivateSubnetEditRoutes.PNG)
...
2. We'll now go through several configuration steps. Start with a Standard create for this MySQL-Compatible Amazon Aurora database. Leave the rest of the defaults in the Engine options as default.
   ...
   Under the Templates section choose Dev/Test since this isn't being used for production at the moment. Under Settings set a username and password of your choice and note them down since we'll be using password authentication to access our database. Select Aurora Standard in Cluster Storage Configuration and in Instance Configuration select "db.r5.large" (according to use case here we are just required practice purpose so we selected lowest one)

   ![](/demos/DBConfig2.PNG)
   ![](/demos/DBConfigCluster.PNG)
...
5. For Configure group size and scaling policies, set desired, minimum and maximum capacity to 2. Click skip to review and then Create Auto Scaling Group.
   ...
   ![](/demos/ConfigureASG4.PNG)

6. Now select the created auto scaling group, Click on Automatic Scaling section as shown below and click on Create dynamic scaling policy.
   ![](/demos/ConfigureASG5.PNG)

7. Select policy type as Target tracking policy, Give name to policy and Select Metric Type as "Average CPU Utilization" and give Target Value as "50" (So if average CPU utilization of app tier servers goes above 50 percent then auto scaling group will create server) and Click on Create.

 ![](/demos/ConfigureASG6.PNG)
...
2. Name the Launch Template, and then under **Application and OS Images** include the app tier AMI you created.
   ...
   ![](</demos/WebServerASGLauchTemplate1.PNG>)
   ![](</demos/WebServerASGLauchTemplate2.PNG>)
